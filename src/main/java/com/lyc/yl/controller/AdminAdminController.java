...
import com.lyc.yl.util.BCryptPasswordEncoder;
...
import static com.lyc.yl.result.AdminResponseCode.*;

/**
 * 管理员表(StoreAdmin)表控制层
 *
 * @author makejava
...
public class AdminAdminController {
    ...
    @Autowired
    private AdminLogController logHelper;
    ...
    public Object list(String username,
        ...
        List<StoreAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(StoreAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody StoreAdmin admin) {
        ...
        List<StoreAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        StoreAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody StoreAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody StoreAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
